...
class Station extends Database{
     ...
     */
    function insert($point_pick, $point_drop, $location_id, $status){
        $sql = parent::$connection->prepare("INSERT INTO `station`(`point_pick`, `point_drop`, `location_id`, `status`) VALUES (?,?,?,?)");
        $sql->bind_param('ssii', $point_pick, $point_drop, $location_id, $status);
        return $sql->execute();
    }
    /**
     * function update station by id
     */
    function update($id, $point_pick, $point_drop, $status, $location_id){
        $sql = parent::$connection->prepare("UPDATE `station` SET `point_pick`= ? ,`point_drop`= ? , `status` = ?, `location_id` = ? WHERE `id` = ?");
        $sql->bind_param('ssiii', $point_pick, $point_drop, $status, $location_id, $id);
        return $sql->execute();
    }
    /**
