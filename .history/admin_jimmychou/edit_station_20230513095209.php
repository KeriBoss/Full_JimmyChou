        ...
        <div class="p-4 d-flex justify-content-start align-items-center">
            <form action="<?= $url ?>/action/location/edit_location.php" method="post" class="w-50" enctype="multipart/form-data">
                ...
                <div class="form-group">
                                ...
                                <input value="<?=$getStationById[0]['point_pick']?>" class="form-control" list="GFGOptions" id="GFGDataList" name="point_pick"
                            ...
                            <div class="form-group">
                                ...
                                <div class="g-location">
                                <input value="<?=$getAllStation[0]['point_drop']?>" class="form-control" list="option_drop" id="GFGDataList" name="point_drop"
                                    placeholder="Chọn địa điểm ..." required>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="departure_time" id="time_pick" placeholder="Chọn giờ khởi hành" pattern="[0-9.0-9]+:[0-9.0-9]+[a-z]{2}" value="<?=$item['departure_time']?>">
                            </div>
                            <div class="form-group">
                                <select name="location_id" class="form-control">
                                    <?php
                                    foreach($getAllLocation as $item){
                                        if($item['id'] == $getAllStation[0]['location_id']){?>
                                            <option value="<?=$getAllStation[0]['location_id']?>" selected>Từ: <?=$item['location_pick']?>, đến: <?=$item['location_drop']?></option>
                                        <?php }else{ ?>
